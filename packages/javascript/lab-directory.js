// 实验室目录数据（所有实验室的统一注册表）
// 各实验室的 README 和 PoC 从这里读取

import { Lab, Poc } from './poc-models.js';
...
export function getLabs() { return labs; }

export function getLab(id) { return labs.find(l => l.id === id); }

export function getLabsByDomain(domain) {
  return domain ? labs.filter(l => l.domain === domain) : labs;
}

export function getLabsByStatus(status) {
  return status ? labs.filter(l => l.status === status) : labs;
}
